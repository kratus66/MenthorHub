...
const CursoDetalle = () => {
   ...
   const userIsEnrolled = curso?.students?.some((student) => student.id === user?.id);
      ...
      <div className="flex justify-center items-center min-h-screen bg-gray-200">
         <div className="max-w-6xl w-full h-screen bg-gray-100 shadow-lg rounded-lg p-8 px-0 m-4 lg:m-0 flex flex-col gap-6">
            <div className="w-full flex flex-col items-center">
               ...
               >
                  {loadingEnroll ? 'Inscribiendo...' : userIsEnrolled ? 'Inscripto' : 'Inscribirme'}
               </button>
            ...
            {taskDetailModalOpen && selectedTask && (
               <div className="fixed inset-0 bg-gray-900 bg-opacity-60 flex justify-center items-center z-50">
                  <div className="bg-white rounded-lg shadow-lg w-2/5 max-w-xl p-6 relative">
                     ...
                     <p className="mb-4 whitespace-pre-wrap">{selectedTask.instructions}</p>
                     <p className="mb-2"><strong>Fecha de entrega:</strong> {new Date(selectedTask.dueDate).toLocaleString()}</p>
  ...
  <section className="p-4 grid grid-cols-1 md:grid-cols-2 gap-4 items-start">
    ...
    <div>
      ...
      {curso.category.imageUrl && (
        <div className="mt-4">
          <h4 className="text-lg font-semibold mb-2">Contenido Multimedia</h4>
          <video
...
{activeTab === 'profesor' && curso.teacher && (
  <div className="flex flex-col items-center justify-center h-full space-y-4 text-center">
    ...
    />
    <p className="text-gray-700 text-lg font-semibold">Nombre del profesor:</p>
    <p className="text-gray-900 text-xl font-bold">{curso.teacher.name}</p>
    <p className="text-gray-700 text-lg font-semibold mt-4">Email de contacto:</p>
    <p className="text-gray-900 text-xl font-bold">{curso.teacher.email}</p>
  </div>
)}
            </div>
