...
interface SubmissionType {
   ...
   student: {
      // Assuming student info is included in the submission for the teacher view
      id: string;
...
async function fetchSubmissions(
   user: { id: string; role: string; name: string; isPaid?: boolean } | null, // Updated user type
   selectedTask: any,
   ...
   try {
      ...
      if (user?.role === 'student') {
         ...
         const response = await axiosInstance.get(
            `/submissions/task/${selectedTask.id}`
         );
...
const CursoDetalle = () => {
   ...
   const [user, setUser] = useState<{
      ...
      role: string;
      name: string;
      isPaid?: boolean; // Added isPaid to user state type
   } | null>(null);
   ...
   useEffect(() => {
      ...
      if (storedUserString) {
         ...
         // Optimistically set user from localStorage first for faster UI response
         setUser(localUser);

         // Then, fetch fresh user data from the server to get the latest isPaid status
         if (localUser && localUser.id) { // Ensure localUser and localUser.id exist
            axiosInstance.get(`/users/${localUser.id}`) // Use the correct endpoint
               .then(response => {
                  const serverUser = response.data;
                  // Update user state with fresh data from server, including isPaid
                  setUser(serverUser);
                  // Optional: Update localStorage if necessary, though this might cause re-renders.
                  // localStorage.setItem('user', JSON.stringify(serverUser));
               })
               .catch(error => {
                  console.error("Error fetching user profile:", error);
                  // If the call fails, the user state will retain data from localStorage.
                  // Consider how to handle this error (show message, restrict access, etc.)
               });
         }
      } else {
         setUser(null); // No user in localStorage
      }
   }, [classId, reviewSent]); // reviewSent can also trigger this effect. Consider if user data needs reloading then.

   const userIsStudent = user?.role === 'student';
   const userIsEnrolled = curso?.students?.some(
      (student: { id: string }) => student.id === user?.id
   );

   const filteredTabs = tabs.filter((tab) => {
      // Hide 'Tareas' tab for students not enrolled
      if (tab.id === 'tareas' && userIsStudent && !userIsEnrolled) {
         return false;
      }
      // Hide 'Multimedia' tab for students who have not paid (using user.isPaid)
      if (tab.id === 'multimedia' && userIsStudent && !user?.isPaid ) {
         return false;
      }
      // Hide 'Tareas' tab for students who have not paid (using user.isPaid)
      if (tab.id === 'tareas' && userIsStudent && !user?.isPaid) {
         return false;
      }
      // Hide 'Multimedia' tab for students who have not paid (using user.isPaid)
      if (tab.id === 'multimedia' && userIsStudent && !userIsEnrolled    ) {
         return false;
      }
      return true;
   });
   ...
   const handleEnroll = async () => {
      ...
      // Check if the user is a student and if user.isPaid is not true
      if (user.role === 'student' && !user.isPaid) {
         alert('Debes completar tu pago para inscribirte a clases.');
         return; // Prevent enrollment if not paid
      }
      ...
      try {
         ...
         // Optionally, re-fetch user data to ensure isPaid status is up-to-date if enrollment affects it
         // For example, by calling the user fetch logic again or updating locally if the backend indicates.
      } catch (error) {
   ...
   const handleSaveGrades = async () => {
      ...
      try {
         await axiosInstance.put(
            `/submissions/task/${selectedTask.id}/grades`,
            {
   ...
   const foundSubmission = submissions.find(
      (sub) => String(sub.task?.id) === String(selectedTask?.id)
   );
   ...
   // Función que llama al endpoint para convertir el texto y reproducir el audio
   const handleSpeech = async () => {
      try {
         ...
         // Convierte el string Base64 en un Blob y luego en una URL para reproducirlo
       ...
       <div className="max-w-6xl w-full h-screen bg-gray-100 shadow-lg rounded-lg p-8 px-0 m-4 lg:m-0 flex flex-col gap-6">
         <div className="w-full flex flex-col items-center">
           ...
           {userIsStudent && !userIsEnrolled && (
             <button
               className="mt-2 w-1/6 bg-blue-500 text-white py-2 rounded-lg font-semibold text-md disabled:opacity-50"
               ...
               <form onSubmit={handleSubmit}>
                 ...
                 >
                   ...
                   {[...Array(24)].map((_, hour) => {
                     ...
                     const shouldDisable =
                       isToday && hour < parseInt(minTime.slice(0, 2));
                     return (
                       <option
                         key={hour}
                         value={hourStr}
                             ...
                             {teacherSubmissions.map((submission) => (
                               ...
                               >
                                 <p className="font-semibold text-gray-800">
                                   Estudiante: {submission.student.name}
                                 </p>
                                 <p className="text-sm text-gray-600 mt-1">
                                   Archivo:{" "}
                                   <a
         ...
         <div className="flex flex-col gap-6 overflow-y-auto">
           ...
           {activeTab === "multimedia" && user.isPaid && (
             <section className="p-4">
               ...
               {curso.multimedia && curso.multimedia.length > 0 ? (
                   ...
                   {curso.multimedia.map((mediaItem, index) => {
                     ...
                     return (
                       ...
                       >
                         ...
                         <span className="text-sm text-gray-700 truncate w-full px-1">
                           {filename}
                         </span>
                       </a>
                     );
                 ...
                 <p className="text-center text-gray-600">
                   No hay contenido multimedia disponible para esta clase.
                 </p>
               )}
             </section>
           )}

           {activeTab === "tareas" && user.isPaid && (
           ...
           {activeTab === "profesor" && curso.teacher && (
             <div className="flex flex-col items-center justify-center h-full space-y-4 text-center">
               <img
                 src={curso.teacher.profileImage || "/default-avatar.png"}
