...
const NavBar: React.FC<NavBarProps> = ({ onSetFiltros }) => {
   ...
   const fetchNotificationsCallback = useCallback(async () => {
      ...
      try {
         const res = await axiosInstance.get<NotificationType[]>(`/notifications/${user.id}`);
      ...
      <>
                  ...
                  <div className="absolute top-10 right-12 bg-white shadow-lg rounded-md w-64 max-h-80 overflow-y-auto z-50">
                     {loadingNotifications ? (
                        ...
                        notifications.map((note, index) => (
                           <div key={note.id || index} className="px-4 py-2 hover:bg-gray-100 text-sm">
                              {note.message}
                           </div>
                        ))
                     ) : (
         ...
         >
            <div className="p-8 mt-16 flex flex-col space-y-6 text-lg md:text-2xl">
               <Link to="/home" onClick={() => setMenuOpen(false)} className="hover:text-gray-200">
                  Home
               </Link>
               ...
               <Link to="/suscripcion" onClick={() => setMenuOpen(false)} className="hover:text-gray-200">
                  Suscríbete
               </Link>
               <Link to="/about-us" onClick={() => setMenuOpen(false)} className="hover:text-gray-200">
                  About Us
               </Link>
            </div>
         </div>
